...
VARIANT_RE = re.compile(r"^([a-z]+)-(\d+)\.(jpg|mp3)$")
...
def collect_variants(directory, ext):
    ...
    for slug, items in sorted(by_slug.items()):
        items.sort()
        rel = "images/animals" if ext == "jpg" else "audio/sounds"
...
def main():
    ...
    slugs = sorted(set(images) | set(sounds))
    manifest = {}
    for slug in slugs:
        manifest[slug] = {
    ...
    print(f"Wrote {MANIFEST_PATH}: {len(manifest)} animals, {total_img} images, {total_snd} sounds", flush=True)
