...
const boot = await page.evaluate(() => {
  const imgs = [...document.querySelectorAll(".reels__illus")];
  return {
    ready: !!window.__reelsReady,
    slides: document.querySelectorAll(".reels__slide:not(.reels__boot-slide)").length,
    videos: document.querySelectorAll("video").length,
    localImages: imgs.filter((i) => i.src.includes("/kids/images/")).length,
    twemojiImages: imgs.filter((i) => i.src.includes("twemoji") || i.src.includes("jsdelivr")).length,
...
const afterTap = await page.evaluate(() => ({
  ...
  illusSrc: document.querySelector(".reels__illus")?.src || null,
}));
...
const ok = boot.ready
  ...
  && boot.videos === 0
  && boot.localImages === 0
  && boot.twemojiImages > 0
  && afterTap.illusSrc?.includes("jsdelivr")
  && configOpen
  && errors.length === 0;
